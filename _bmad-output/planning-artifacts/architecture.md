...
### Decision 5.1: Hosting & Deployment Strategy (Aspire-Native)

**Selected Option: A - Aspire Docker Publisher (MVP) → Aspire Kubernetes Publisher (Phase 2) (Recommended)**

**Selected Technologies (Aspire-Native):**
- **MVP Deployment:** Aspire Docker Publisher (generates Docker Compose via C#)
- **Phase 2 Deployment:** Aspire Kubernetes Publisher (generates K8s manifests)
- **Container Registry:** Docker Hub (public) or Self-Hosted Registry (private)
- **Server OS:** Linux (Ubuntu 22.04 LTS recommended)
- **Aspire Packages:**
  - `Aspire.Hosting.Docker` (NuGet package)
  - `Aspire.Hosting.Kubernetes` (Phase 2)
  - `ServiceDefaults` (shared configuration across services)

**Aspire-Native AppHost Configuration (Program.cs):**

```csharp
// Program.cs in BmadServer.AppHost project
using Aspire.Hosting;

var builder = DistributedApplication.CreateBuilder(args);

// Add PostgreSQL database with persistence
var postgres = builder
    .AddPostgres("postgres")
    .WithDataVolume()
    .WithInitializationSql(@"
        CREATE DATABASE IF NOT EXISTS bmadserver;
        CREATE USER IF NOT EXISTS bmad WITH PASSWORD 'password';
        ALTER ROLE bmad CREATEDB;
    ");

var bmadDatabase = postgres.AddDatabase("bmadserver");

// Add API service with service defaults (OpenTelemetry, health checks, resilience)
var apiService = builder
    .AddProject<Projects.BmadServer_ApiService>("api")
    .WithReference(bmadDatabase)
    .WithEnvironment("ASPNETCORE_ENVIRONMENT", "Production")
    .WithHttpEndpoint(targetPort: 5000, name: "http") // Named endpoint
    .WithHttpsEndpoint(targetPort: 5001, name: "https")
    .WithHealthCheck("health", "/health", initialDelay: TimeSpan.FromSeconds(5));

// Add Aspire dashboard for telemetry (built-in OpenTelemetry)
builder.AddAspireDashboard()
    .WithMetricsEndpoint(apiService, "/metrics");

// Add Redis cache (Phase 2 scaling)
var cache = builder
    .AddRedis("cache", port: 6379)
    .WithDataVolume()
    .WithHttpsEndpoint(targetPort: 6379, name: "https");

// Wire up cache to API service
apiService.WithReference(cache);

// Build and configure distribution
var app = builder.Build();

// Configure Docker Compose publisher (MVP)
app.AddDockerComposePublisher()
    .WithPushImageToRegistry(pushImages: false) // Local registry
    .WithDockerComposePublisher();

// Configure Kubernetes publisher (Phase 2) - commented out for MVP
// app.AddKubernetesEnvironment("k8s")
//    .WithHelm()
//    .WithoutTls();
...
**Service Defaults Configuration (ServiceDefaults):**
...
namespace BmadServer.ServiceDefaults;
...
public static class AspireExtensions
{
    public static IHostBuilder AddServiceDefaults(this IHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            ...
            // OpenTelemetry (automatic via Aspire)
...
**Deployment Topology (Aspire-Native MVP):**

```
┌──────────────────────────────────────────────────────────┐
│  Local Development Machine                               │
│  (AppHost runs: dotnet run)                              │
├──────────────────────────────────────────────────────────┤
│  Aspire Dashboard (UI)                                   │
│  • Logs, Metrics, Traces (OpenTelemetry OTLP)           │
...
│  │  • Health check: GET /health (via Aspire)         │ │
│  │  • Metrics endpoint: GET /metrics                 │ │
│  │  • Service discovery enabled                      │ │
│  │  • OpenTelemetry OTLP exporter → Aspire Dashboard │ │
│  └────────────────────────────────────────────────────┘ │
│           ↓                                              │
│  ┌────────────────────────────────────────────────────┐ │
│  │  Service: postgres (PostgreSQL 17)                 │ │
│  │  • Connection pooling: PgBouncer                  │ │
│  │  • Volume: postgres_data                          │ │
│  │  • Health check: pg_isready (via Aspire)         │ │
│  │  • Backups: hourly WAL + daily snapshots         │ │
│  └────────────────────────────────────────────────────┘ │
│           ↓                                              │
│  ┌────────────────────────────────────────────────────┐ │
│  │  Service: cache (Redis 7) - Phase 2               │ │
│  │  • In-memory caching                              │ │
│  │  • Session persistence                            │ │
...
│                                                          │
│  ┌────────────────────────────────────────────────────┐ │
│  │  Service: aspire-dashboard (standalone OTLP)      │ │
...
**Deployment Process (Aspire-Native):**

1. **Local Development (Aspire Dashboard):**
   ```bash
   cd src/BmadServer.AppHost
   dotnet run
   # Opens Aspire Dashboard at http://localhost:18888
   # Real-time metrics, logs, traces, service discovery
   ```
...
3. **Deploy to Production Server:**
   ...
   scp -r docker-compose-artifacts username@server:/opt/bmadserver/
   ssh username@server
   cd /opt/bmadserver
   ...
   export ASPIRE_DASHBOARD_PASSWORD=$(openssl rand -base64 24)
...
4. **Verify Production Deployment:**
   ...
   curl https://bmadserver.local/metrics
   
   # Aspire Dashboard (standalone mode)
   # Access at: https://bmadserver.local:18888 (internal only)
   ```

**Key Aspire-Native Features Leveraged:**

1. **Service Discovery (Built-in):**
   - API automatically discovers PostgreSQL endpoint
   - No hardcoded hostnames
   - Works identically in dev/prod

2. **Health Checks (Built-in):**
   - AppHost monitors resource readiness
   - Dependency startup ordering automatic
   - Docker health checks auto-configured

3. **OpenTelemetry Integration (Built-in):**
   - Metrics, Logs, Traces collected automatically
   - No manual instrumentation needed
   - OTLP exporter to Aspire Dashboard
   - Prometheus scraping endpoint auto-generated

4. **Configuration Management:**
   - AppHost C# code declares all config
   - Aspire generates environment variables
   - Secrets managed via .env files
   - No manual docker-compose.yml editing

5. **Scaling Path:**
   - AppHost can generate Kubernetes manifests
   - Same code → different deployment target
   - ServiceDefaults apply across all environments

**Rationale:**

1. **Aspire-Native Advantages:**
   - Infrastructure declared in C# (type-safe)
   - No manual Docker Compose maintenance
   - Built-in observability (OpenTelemetry)
   - Service discovery automatic
   - Health checks configured by Aspire

2. **Development-to-Production Parity:**
   - Same AppHost runs in dev (Dashboard) and prod (Docker)
   - Same service discovery configuration
...
3. **Self-Hosted Benefits:**
   ...
   - No vendor lock-in
   - Cost predictable (servers only)
   - Aspire-generated manifests (Kubernetes phase 2)

**Alternatives (Not Selected):**
- **Manual docker-compose.yml** - Maintenance burden, no type safety
- **Azure Container Apps** - Cloud vendor lock-in
- **Kubernetes from Day 1** - Operational complexity for MVP
- **Serverless** - Incompatible with persistent WebSocket + database
...
### Decision 5.4: Monitoring & Logging Strategy (Aspire-Native OpenTelemetry)

**Selected Option: A - Aspire Dashboard (MVP) → Prometheus + Grafana (Production) (Recommended)**

**Selected Technologies (Aspire-Native):**
- **Metrics:** OpenTelemetry (Aspire built-in) → Prometheus export
- **Visualization (Dev):** Aspire Dashboard (built-in, OTLP endpoint)
- **Visualization (Prod):** Prometheus + Grafana (self-hosted)
- **Distributed Tracing:** OpenTelemetry OTLP (built-in)
- **Logging:** Structured JSON to stdout (Docker captures)
- **Packages:**
  - `Aspire.Hosting.Dashboard` (MVP local development)
  - `Aspire.Dashboard.Standalone` (production OTLP collector)

**Aspire-Native Monitoring Architecture:**

```
┌──────────────────────────────────────────────────────────┐
│  LOCAL DEVELOPMENT (AppHost + Dashboard)                │
├──────────────────────────────────────────────────────────┤
│                                                          │
│  dotnet run (BmadServer.AppHost)                        │
│  ├─ Starts: API service                                │
│  ├─ Starts: PostgreSQL                                 │
│  ├─ Starts: ServiceDefaults (OpenTelemetry collection) │
│  └─ Launches: Aspire Dashboard (port 18888)           │
│                                                          │
│  ┌──────────────────────────────────────────────────┐  │
│  │  Aspire Dashboard (Real-time telemetry)         │  │
│  │  • Logs (structured JSON)                       │  │
│  │  • Metrics (live graphs)                        │  │
│  │  • Traces (distributed tracing)                 │  │
│  │  • Service discovery (endpoints, config)        │  │
...
│  │  • Automatic instrumentation (ServiceDefaults)  │  │
│  │  • OpenTelemetry OTLP exporter                  │  │
│  │  • Metrics endpoint: GET /metrics               │  │
│  │  • Health endpoint: GET /health                 │  │
│  │  • Structured logging to stdout (JSON)          │  │
│  │  • Export to: aspire-dashboard:4317 (OTLP)     │  │
│  └──────────────────────────────────────────────────┘  │
│               ↓ (OTLP gRPC)                             │
│  ┌──────────────────────────────────────────────────┐  │
│  │  Aspire Dashboard (Standalone Container)        │  │
│  │  • OTLP receiver (port 4317)                    │  │
│  │  • Real-time logs, metrics, traces              │  │
│  │  • Web UI for operational viewing                │  │
│  └──────────────────────────────────────────────────┘  │
│               ↓ (Optional export)                       │
│  ┌──────────────────────────────────────────────────┐  │
...
**Aspire ServiceDefaults (Built-in OpenTelemetry):**
...
public static class AspireExtensions
{
    public static IHostBuilder AddServiceDefaults(this IHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            ...
            // Health checks (auto-wired by Aspire)
            services.AddHealthChecks()
                .AddDbContextCheck<AppDbContext>()
            ...
            // - Metrics: ASP.NET Core + HTTP Client
            // - Traces: ASP.NET Core + HTTP Client + Database
            // - Logs: Structured JSON
            // - OTLP exporter to Aspire Dashboard
            services.AddOpenTelemetry()
                .WithMetrics(metrics =>
...
**Aspire Dashboard (docker-compose.yml - generated):**

```yaml
services:
  # Aspire Dashboard (Standalone OTLP collector)
  aspire-dashboard:
    image: mcr.microsoft.com/dotnet/aspire-dashboard:latest
    environment:
      ASPIRE_ALLOWED_HOSTS: "*"
      OTEL_EXPORTER_OTLP_ENDPOINT: http://localhost:4317
    ports:
      - "18888:18888"   # Dashboard UI (internal only)
...
**Prometheus Configuration (auto-generated from Aspire):**

```yaml
# prometheus.yml (generated by aspire publish)
global:
  scrape_interval: 15s
...
**Key Aspire-Native Features:**

1. **Automatic Instrumentation (ServiceDefaults):**
   - No manual metric definitions needed
   - Aspire instruments standard .NET metrics automatically
   - ASP.NET Core, HTTP Client, Database, Runtime metrics
   - Single line to enable: `.AddServiceDefaults()`

2. **OpenTelemetry Built-In:**
   - OTLP exporter configured by default
   - Sends to Aspire Dashboard automatically in production
   - Optional Prometheus scraping endpoint
   - Traces propagation automatic across services

3. **Development-to-Production Parity:**
   - Local: Aspire Dashboard (real-time telemetry)
   - Production: Same metrics exported via OTLP
   - No code changes for environment differences
   - Aspire Dashboard also available in standalone mode

4. **Zero-Configuration Monitoring:**
   - Health checks auto-configured by AppHost
   - Service discovery automatic
   - Resilience policies included
   - Structured logging built-in

**Rationale:**

1. **Aspire Dashboard Benefits:**
   - Real-time debugging during development
   - No additional tooling needed locally
   - Full trace visibility for troubleshooting
   - Configuration inspection (environment variables)

2. **Production Flexibility:**
   - Export to Prometheus for long-term storage
   - Use Grafana for historical analysis + alerts
   - Or use Aspire Dashboard standalone for real-time
   - Can use both simultaneously

3. **Self-Hosted Control:**
   - All telemetry stays within your infrastructure
   - No external observability platform needed
   - Aspire Dashboard = lightweight container
   - Prometheus + Grafana = standard open-source stack

**Alternatives (Not Selected):**
- **Application Insights** - Cloud vendor lock-in (Azure)
- **DataDog** - Expensive per-host pricing
- **No observability** - Impossible to troubleshoot production issues
        version: v11
        index:
          prefix: index_
          period: 24h
